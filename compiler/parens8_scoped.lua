...
function eval(exp, lookup)
	...
	if exp[1] == "fn" then
		...
		for i,v in ipairs(exp[2]) do locals[v] = i end
		local compiled = eval(exp[3], function(name)
	...
	local n, op, compiled = #exp, builtin[exp[1]], {}
...
function builtin:when(lookup, a1, a2, a3)
	return function(...)
		if (a1(...)) return a2(...)
		if (a3) return a3(...)
	end
